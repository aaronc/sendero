...
@cyclic("test.senderoxc.test1")+/;

/+@data+/ class TestCyclic
: IObject, IHttpSet
...
void httpSet(IObject obj, Request req)
{
	foreach(key, val; obj)
	{
		switch(key)
		{
			case "user": user = convertParam2!(User, Req)(val); break;
			default: break;
		}
	}
}
...
public void destroy()
{
	if(!deleteSql.length) deleteSql = db.sqlGen.makeDeleteSql("TestCyclic", ["id"]);
	scope st = db.prepare(deleteSql);
	st.execute(id_);
}


public User user() {return user_;}
public void user(User val) {__touched__[0] = true; user_ = val;}
private HasOne!(User) user_.get;
